...
def gen_adj_mat_tensor(data, parameter, metric="cosine", armed_gpu = 0):
    assert metric == "cosine", "Only cosine distance implemented"
    dist = cosine_distance_torch(data, data)
...
def gen_test_adj_mat_tensor(data, trte_idx, parameter, metric="cosine", armed_gpu = 0):
    assert metric == "cosine", "Only cosine distance implemented"
    adj = torch.zeros((data.shape[0], data.shape[0]))
    if cuda:
        adj = adj.cuda(device = armed_gpu)
...
def load_model_dict(folder, model_dict, armed_gpu = 0):
    for module in model_dict:
...
#            print("Module {:} loaded!".format(module))
        ...
        if cuda:
            model_dict[module].cuda(device = armed_gpu)    
    return model_dict